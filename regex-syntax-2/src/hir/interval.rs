...
impl<I: Interval> IntervalSet<I> {
    ...
    ///
    /// If the intervals given aren't in a canonical ordering, then they are
    /// converted into a canonical ordering.
    pub fn new<T: Into<Vec<I>>>(intervals: T) -> IntervalSet<I> {
        let mut set = IntervalSet { ranges: intervals.into() };
        set.canonicalize();
        set
    }
    ...
    /// Consume this set and return the underlying sequence of intervals.
    ///
    /// The sequence returned is in canonical ordering.
    pub fn into_intervals(self) -> Vec<I> {
        self.ranges
    }
