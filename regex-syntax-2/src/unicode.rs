...
impl<'a> ClassQuery<'a> {
    fn canonicalize(&self) -> Result<CanonicalClassQuery> {
        match *self {
            ...
            ClassQuery::ByValue { property_name, property_value } => {
                ...
                Ok(match canon_name {
                    "General_Category" => {
                        CanonicalClassQuery::GeneralCategory(canon_val)
                    }
                    "Script" => {
                        CanonicalClassQuery::Script(canon_val)
                    }
                    _ => {
    ...
    fn canonical_binary(&self, name: &str) -> Result<CanonicalClassQuery> {
        if let Some(canon) = canonical_prop(name) {
            return Ok(CanonicalClassQuery::Binary(canon));
        }
        let gencats = property_values("General_Category").unwrap();
        if let Some(canon) = canonical_value(gencats, name) {
            return Ok(CanonicalClassQuery::GeneralCategory(canon));
        }
        let scripts = property_values("Script").unwrap();
        if let Some(canon) = canonical_value(scripts, name) {
...
pub fn hir_class(ranges: &[(char, char)]) -> hir::ClassUnicode {
    let hir_ranges: Vec<hir::ClassRangeUnicode> = ranges
        .iter()
...
fn canonical_prop(name: &str) -> Option<&'static str> {
    let mut name = name.to_string();
    ucd_util::symbolic_name_normalize(&mut name);
    ucd_util::canonical_property_name(PROPERTY_NAMES, &name)
}

fn canonical_value(vals: PropertyValues, value: &str) -> Option<&'static str> {
    let mut value = value.to_string();
    ucd_util::symbolic_name_normalize(&mut value);
    ucd_util::canonical_property_value(vals, &value)
}
